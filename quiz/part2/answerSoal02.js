...
function checkPalindrome(num) {
  ...
  let wordLen = numToString.length - 1;
  let reversedWord = "";
  for (let i = wordLen; i >= 0; i--) {
    reversedWord += numToString[i];
  }

  if (reversedWord === numToString) {
    return true;
  } else if (reversedWord !== numToString) {
    return false;
  }
}
...
function angkaPalindrome(num) {
  ...
  while (!isPalindrome) {
    result += 1;
    const newNum = checkPalindrome(result);
    if (newNum) {
      isPalindrome = true;
    }
  }
